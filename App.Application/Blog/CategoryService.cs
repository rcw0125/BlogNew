...
namespace App.Application.Blog
{
    public class CategoryService : AppService<CategoryInfo>, ICategoryService
    {
        ...
        public async Task<List<CategoryDto>> GetRootCategoriesByCid(string cid)
        {
            string ParentId = cid;
            if (cid != "0")
            {
                List<CategoryInfo> childList = GetList(x => x.ParentId == cid);
                if (childList.Count == 0 || childList == null)
                {
                    CategoryInfo curCategoryInfo = FindEntity(cid);
                    ParentId = curCategoryInfo.ParentId;
                }
            }
            return await GetListAsync<CategoryDto>(x => x.ParentId == ParentId);
        }
    }
}